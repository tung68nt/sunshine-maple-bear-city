    ...
    <section id="gallery" className="py-24 lg:py-32 bg-white relative border-b border-neutral-200/60">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-4 mb-12">
          <span className="text-xs font-display font-black text-maple-red uppercase tracking-[0.2em]">04 / EXHIBITION</span>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-display font-extrabold text-[#1D1D1B]">
              Campus <span className="font-serif italic font-normal text-maple-red">Gallery</span>
            </h2>
            <p className="text-sm text-neutral-500 font-light mt-2">
              Explore our world-class facilities designed for active learning and well-being.
            </p>
          </div>

          {/* Sharp Controls Box */}
          <div className="flex items-center gap-4 border border-neutral-300 p-1.5 bg-[#FDFBF7]">
            <button
              onClick={prevSlide}
              aria-label="Previous"
              className="w-9 h-9 border border-neutral-200 bg-white hover:bg-[#1D1D1B] hover:text-white flex items-center justify-center transition-colors text-xs font-bold"
            >
              ←
            </button>
          ...
          {visibleItems.map((item, idx) => {
            ...
            return (
              <div
                ...
                className="border border-neutral-300 bg-white p-2.5 shadow-sm group cursor-pointer hover:shadow-xl transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-maple-red"
              >
                <div className="relative h-80 sm:h-96 overflow-hidden bg-neutral-200 animate-pulse group-hover:animate-none">
                  <Image
                    src={item.src}
                    alt={item.title}
                    fill
                  ...
                  <div className="absolute top-4 left-4 z-10">
                    <span className="px-3 py-1 bg-black/60 backdrop-blur-md text-white text-[10px] font-display font-bold tracking-widest border border-white/20">
                      {item.category}
                    </span>
                  </div>

                  {/* Expand Icon */}
                  <div className="absolute top-4 right-4 z-10 w-8 h-8 bg-black/60 text-white flex items-center justify-center border border-white/20 opacity-0 group-hover:opacity-100 transition-opacity">
                    <Maximize2 size={14} />
                  </div>
      ...
      {lightboxIndex !== null && (
        <div className="fixed inset-0 z-[9999] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4">
          <button
            ...
            aria-label="Close"
            className="absolute top-6 right-6 text-white/80 hover:text-white bg-white/10 p-3 transition-colors z-50 border border-white/20"
          >
            <X size={24} />
          </button>
          ...
          <button
            ...
            aria-label="Previous"
            className="absolute left-4 top-1/2 -translate-y-1/2 text-white/80 hover:text-white bg-white/10 p-4 transition-colors z-50 border border-white/20"
          >
            <ChevronLeft size={28} />
          </button>
          ...
          <div className="relative max-w-5xl w-full h-[75vh]">
            <Image
              ...
              sizes="100vw"
            />
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/80 text-white px-6 py-2 border border-white/20 text-xs font-display font-bold uppercase tracking-wider">
          ...
          <button
            ...
            aria-label="Next"
            className="absolute right-4 top-1/2 -translate-y-1/2 text-white/80 hover:text-white bg-white/10 p-4 transition-colors z-50 border border-white/20"
          >
